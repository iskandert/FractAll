...
**Цель:** веб-приложение с модулем для генерации L-систем (строковые переписывания) и их визуализации на холсте. Архитектура: *чистые функции + «команды как данные»*, воркер-пайплайн для тяжёлых расчётов, flat-буферы для производительного рендеринга, Pinia для сериализуемого состояния (shallowRef для больших буферов).
