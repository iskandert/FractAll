...
3. UI предлагает два действия:
   ...
   * *Preview*: вызывает `store.preview(settings)` или `service.preview(settings)` (preview делает быстрый run, малое число итераций, без heavy buffers и расчетов промежуточных итераций).
   * *Apply* (full): вызывает `store.apply(settings)` — запускает generation (в main thread для малых задач или в worker для heavy, с просчетом списка команд для каждого элемента истории).
...
* *Не кладите в store прямые большие массивы реактивно.* Вместо этого храните `shallowRef` на `FlatVertices` или храните только meta+pattern and a pointer to buffers in a service holder`.
